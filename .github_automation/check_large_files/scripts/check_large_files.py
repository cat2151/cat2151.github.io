...
def create_output_files(large_files: List[Dict[str, Any]], config: Dict[str, Any], output_dir: str):
    ...
    # Write issue title
    title_template = config.get("settings", {}).get(
        "issue_title", "大きなファイルの検出: {count}個のファイルが{max_lines}行を超えています"
    )
    title = title_template.format(count=len(large_files), max_lines=config["settings"]["max_lines"])
    ...
    # Write summary JSON
    summary = {"count": len(large_files), "max_lines": config["settings"]["max_lines"], "files": large_files}
